...
/**
 * Using a completely custom comparison function here, as `String.localeCompare` does not
 * support sorting characters with all uppercase letters before lowercase letters, which
 * is the desired behavior for a case-sensitive import sort. When `sensitivity` is set to
 * `base`, `String.localeCompare` just sorts by alphabetical *then* by case, but we want
 * to sort by case first, then alphabetical.
 */
